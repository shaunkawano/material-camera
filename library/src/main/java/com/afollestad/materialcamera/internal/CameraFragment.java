...
public class CameraFragment extends BaseCameraFragment implements View.OnClickListener {
  ...
  @Override public void openCamera() {
    ...
    try {
          ...
          if (getArguments().getBoolean(CameraIntentKey.DEFAULT_TO_FRONT_FACING, false)) {
            ...
            if (mInterface.getFrontCamera() != null
              ...
              if (mInterface.getBackCamera() != null
                  && (Integer) mInterface.getBackCamera() != -1) {
      ...
      Camera.Size previewSize =
          chooseOptimalSize(parameters.getSupportedPreviewSizes(), mWindowSize.x, mWindowSize.y,
              mVideoSize);
  ...
  private Camera.Size getHighestSupportedStillShotSize(List<Camera.Size> supportedPictureSizes) {
    Collections.sort(supportedPictureSizes, new Comparator<Camera.Size>() {
      @Override public int compare(Camera.Size lhs, Camera.Size rhs) {
  ...
  private void setCameraDisplayOrientation(Camera.Parameters parameters) {
    ...
    mDisplayOrientation = Degrees.getDisplayOrientation(info.orientation, deviceOrientation,
        info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT);
    Log.d("CameraFragment",
        String.format("Orientations: Sensor = %d˚, Device = %d˚, Display = %d˚", info.orientation,
            deviceOrientation, mDisplayOrientation));
  ...
  private boolean prepareMediaRecorder() {
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        canUseAudio = ContextCompat.checkSelfPermission(activity, Manifest.permission.RECORD_AUDIO)
        ...
        mMediaRecorder.setOnInfoListener(new MediaRecorder.OnInfoListener() {
          @Override public void onInfo(MediaRecorder mediaRecorder, int what, int extra) {
            if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED) {
              Toast.makeText(getActivity(), R.string.mcam_file_size_limit_reached,
  ...
  @Override public boolean startRecordingVideo() {
  ...
  @Override public void stopRecordingVideo(final boolean reachedZero) {
  ...
  @Override public void takeStillshot() {
